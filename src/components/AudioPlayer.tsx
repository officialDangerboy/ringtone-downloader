      ...
      <Card className="fixed bottom-0 left-0 right-0 z-50 glass-card border-t border-border/50 backdrop-blur-xl">
        <div className="max-w-screen-2xl mx-auto px-3 md:px-4 py-2 md:py-4">
          ...
          <div className="sm:hidden mb-2 px-1">
            ...
            >
              <div
                className="absolute left-0 top-0 h-full bg-gradient-to-r from-primary/90 via-primary to-primary/80 rounded-full transition-all"
                style={{ width: `${(currentTime / duration) * 100}%` }}
              />

              {/* Handle */}
              <div
                className="absolute top-1/2 -translate-y-1/2 h-4 w-4 bg-primary shadow-md 
          ...
          <div className="flex items-center gap-3 md:gap-6">
            ...
            <div className="flex items-center gap-2 md:gap-4">
              <button onClick={onPrevious} className="hidden sm:flex w-8 h-8 md:w-10 md:h-10 items-center justify-center rounded-full hover:bg-muted">
                <SkipBack className="w-4 h-4 md:w-5 md:h-5" />
              </button>

              <button
                onClick={onPlayPause}
                className="w-10 h-10 md:w-12 md:h-12 bg-primary rounded-full flex items-center justify-center
                hover:scale-110 transition-transform shadow-lg"
              >
                {isPlaying ? <Pause className="w-5 h-5 md:w-6 md:h-6 text-primary-foreground" /> : <Play className="w-5 h-5 md:w-6 md:h-6 text-primary-foreground ml-0.5" />}
              </button>

              <button onClick={onNext} className="hidden sm:flex w-8 h-8 md:w-10 md:h-10 items-center justify-center rounded-full hover:bg-muted">
